...
# 导入控制器
from desktop.controllers.audio_controller import AudioController
from desktop.controllers.video_controller import VideoController
...
class MainWindow(QMainWindow):
    ...
    def on_audio_generation_finished(self, success: bool):
        """音频生成成功完成的回调"""
        if success:
            self.statusBar().showMessage("语音生成完成")
            # 更新界面
            for row in range(self.content_layout.rowCount()):
                if row < len(self.image_items):
                    item = self.image_items[row]
                    if item.has_audio:
                        # 启用试听按钮
    ...
    def on_audio_generation_error(self, error_msg: str):
        """音频生成错误的回调"""
        QMessageBox.warning(self, "错误", f"语音生成失败: {error_msg}")
        self.progress_dialog.close()
    ...
    def preview_video(self, video_path=None):
        """预览视频"""
        self.video_controller.preview_video(video_path)
    ...
    def generate_clip(self, item: ImageItem):
        ...
        if not hasattr(item, 'animation') or not item.animation:
            # 获取当前行的动画设置组件
            row = self.image_items.index(item)
